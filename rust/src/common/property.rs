...
pub mod guri_h {
    ...
    pub type GUriHideFlags = std::ffi::c_uint;
...
pub mod property_h {
    ...
    use super::gtypes_h::{gchar, gboolean, gdouble, gfloat, gint};
...
pub use self::gtypes_h::{gchar, gint, gboolean, guint, gfloat, gdouble, gpointer};
...
pub use self::guri_h::{
    ...
    G_URI_FLAGS_PARSE_RELAXED, G_URI_FLAGS_NONE, GUriHideFlags, G_URI_HIDE_FRAGMENT,
    G_URI_HIDE_QUERY, G_URI_HIDE_AUTH_PARAMS, G_URI_HIDE_PASSWORD, G_URI_HIDE_USERINFO,
    G_URI_HIDE_NONE, _GUri, g_uri_unref, g_uri_parse, g_uri_to_string_partial,
    g_uri_get_scheme, g_uri_get_host, g_uri_get_path,
};
pub use self::gtype_h::{GType, _GTypeClass, GTypeClass, _GTypeInstance, GTypeInstance};
pub use self::gobject_h::{_GObject, GObject, g_object_set, g_object_get};
pub use self::lua_h::{
    lua_Number, lua_State, lua_type, lua_toboolean, lua_pushnumber, lua_pushstring,
    lua_pushfstring, lua_pushboolean, lua_error,
};
pub use self::log_h::{
...
pub use self::property_h::{
    property_value_t, URI, INT, FLOAT, DOUBLE, CHAR, BOOL, property_tmp_t, property_t,
};
...
use self::lauxlib_h::{luaL_typerror, luaL_checklstring, luaL_checknumber};
...
unsafe extern "C" fn luaH_gobject_get(
    ...
    let mut tmp: property_tmp_t = property_tmp_t {
        c: 0 as *mut gchar,
    };
            ...
            if lua_type(L, vidx) == 0 as std::ffi::c_int {
                ...
                u = g_uri_parse(
                    ...
                    (G_URI_FLAGS_HAS_PASSWORD as std::ffi::c_int
                        ...
                        | G_URI_FLAGS_SCHEME_NORMALIZE as std::ffi::c_int) as GUriFlags,
                    0 as *mut *mut GError,
                );
                valid = (u.is_null()
                    || (g_strcmp0(
                        ...
                        ) == 0) && !(g_uri_get_host(u)).is_null()
                        && !(g_uri_get_path(u)).is_null()) as std::ffi::c_int;
                if valid != 0 {
                    g_object_set(
                        object as gpointer,
                        (*p).name,
