...
pub use self::lua_h::{
    lua_Integer, lua_State, lua_settop, lua_pushinteger, lua_pushstring,
};
pub use self::log_h::{
...
pub use self::ipc_h::{
    ...
    IPC_TYPE_lua_require_module, ipc_endpoint_t, _ipc_endpoint_t, ipc_recv_state_t,
    _ipc_recv_state_t, ipc_header_t, _ipc_header_t, ipc_endpoint_status_t,
    IPC_ENDPOINT_FREED, IPC_ENDPOINT_CONNECTED, IPC_ENDPOINT_DISCONNECTED, ipc_send_lua,
};
pub use self::extension_h::{extension_t, _extension_t, extension};
use self::gmem_h::g_free;
use self::gstrfuncs_h::g_strdup_vprintf;
#[unsafe(no_mangle)]
...
pub unsafe extern "C" fn va_log(
    ...
    let mut L = common.L;
    let mut msg = g_strdup_vprintf(fmt, ap.as_va_list());
    lua_pushinteger(L, lvl as lua_Integer);
