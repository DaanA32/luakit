...
#[c2rust::header_src = "/usr/include/gtk-3.0/gtk/gtkspinner.h:20"]
pub mod gtkspinner_h {
    #[derive(Copy, Clone)]
    #[repr(C)]
    #[c2rust::src_loc = "45:8"]
    pub struct _GtkSpinner {
        pub parent: GtkWidget,
        pub priv_0: *mut GtkSpinnerPrivate,
    }
    #[c2rust::src_loc = "43:1"]
...
pub use self::gobject_h::{
    _GObject, GObject, GInitiallyUnowned, g_object_get, g_object_connect,
};
pub use self::lua_h::{lua_CFunction, lua_State, lua_pushcclosure, lua_pushboolean};
...
pub use self::gtkspinner_h::{
    _GtkSpinner, GtkSpinnerPrivate, GtkSpinner, _GtkSpinnerPrivate, gtk_spinner_get_type,
...
use self::lauxlib_h::{luaL_argerror, luaL_error};
...
#[c2rust::src_loc = "22:1"]
unsafe extern "C" fn luaH_checkspinner(
    mut L: *mut lua_State,
    mut udx: gint,
) -> *mut widget_t {
    let mut w = luaH_checkwidget(L, udx);
    if (*(*w).info).tok as std::ffi::c_uint
        != L_TK_SPINNER as std::ffi::c_int as std::ffi::c_uint
    {
        luaL_argerror(
            L,
            udx,
            b"incorrect widget type (expected spinner)\0" as *const u8
                as *const std::ffi::c_char,
        );
    }
    return w;
