...
#[c2rust::header_src = "/usr/include/gtk-3.0/gtk/gtkstack.h:22"]
pub mod gtkstack_h {
    #[derive(Copy, Clone)]
    #[repr(C)]
    #[c2rust::src_loc = "67:8"]
...
pub use self::lua_h::{
    lua_CFunction, LUA_REGISTRYINDEX, LUA_TBOOLEAN, lua_State, lua_remove, lua_type,
    lua_toboolean, lua_pushlstring, lua_pushcclosure, lua_pushboolean,
...
pub use self::gtkcontainer_h::{
    ...
    gtk_container_get_type, gtk_container_add,
};
pub use self::gtkstack_h::{
    _GtkStack, GtkStack, gtk_stack_get_type, gtk_stack_new, gtk_stack_set_visible_child,
    gtk_stack_get_visible_child, gtk_stack_set_homogeneous, gtk_stack_get_homogeneous,
};
...
unsafe extern "C" fn luaH_stack_index(
    ...
    match token as std::ffi::c_uint {
        ...
        254 => {
            let mut widget = gtk_stack_get_visible_child(
                g_type_check_instance_cast(
                    (*w).widget as *mut GTypeInstance,
                    gtk_stack_get_type(),
                ) as *mut std::ffi::c_void as *mut GtkStack,
