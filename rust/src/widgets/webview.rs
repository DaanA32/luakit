...
use crate::LUAKIT_ERROR_TLS;
use crate::clib::luakit::*;
use crate::clib::msg::*;
use crate::clib::request::luaH_request_push_uri_scheme_request;
use crate::clib::soup::*;
...
use crate::ipc::ipc_h::*;
use crate::ipc::*;
use crate::ipc_common::ipc::ipc_endpoint_decref;
use crate::ipc_common::ipc::ipc_endpoint_new;
use crate::ipc_common::ipc::ipc_endpoint_replace;
use crate::log::*;
use crate::luah::luaH_modifier_table_push;
use crate::web_context::web_context_init_finish;
use crate::web_context_get;
use crate::widgets::common::*;
use crate::widgets::webview::auth::session_authenticate;
use crate::widgets::webview::find_controller::failed_to_find_text_cb;
use crate::widgets::webview::find_controller::found_text_cb;
use crate::widgets::webview::inspector::inspector_attach_window_cb;
use crate::widgets::webview::inspector::inspector_close_window_cb;
use crate::widgets::webview::inspector::inspector_detach_window_cb;
use crate::widgets::webview::inspector::inspector_open_window_cb;
use crate::widgets::webview::inspector::inspector_show_window_cb;
use crate::widgets::webview::inspector::luaH_webview_close_inspector;
use crate::widgets::webview::inspector::luaH_webview_show_inspector;
use crate::widgets::webview::javascript::luaH_webview_eval_js;
use crate::widgets::webview::scroll::luaH_webview_push_scroll_table;
use crate::widgets::webview::stylesheets::luaH_webview_push_stylesheets_table;
use crate::widgets::webview::stylesheets::webview_update_stylesheets;
use crate::widgets::*;

pub unsafe extern "C-unwind" fn luaH_webview_push_history(
    mut L: *mut lua_State,
    ...
    let mut backlen = g_list_length(webkit_back_forward_list_get_back_list(bflist)) as lua_Integer;
    let mut forwardlen =
        g_list_length(webkit_back_forward_list_get_forward_list(bflist)) as lua_Integer;
    ...
    lua_pushnumber(L, (backlen + 1) as lua_Number);
    lua_rawset(L, -(3));
    lua_createtable(L, (backlen + forwardlen + 1) as i32, 0);
    let mut i = -backlen;
    while i <= forwardlen {
        item = webkit_back_forward_list_get_nth_item(bflist, i as c_int);
        lua_createtable(L, 0, 2);
        lua_pushlstring(
            L,
...
pub unsafe extern "C-unwind" fn luaH_webview_can_go_back(mut L: *mut lua_State) -> gint {
...
pub unsafe extern "C-unwind" fn luaH_webview_can_go_forward(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    lua_pushboolean(L, webkit_web_view_can_go_forward((*d).view));
    return 1 as std::ffi::c_int;
}
pub unsafe extern "C" fn webview_history_go(mut L: *mut lua_State, mut direction: gint) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    ...
    lua_pushboolean(L, (item != std::ptr::null_mut()) as std::ffi::c_int);
    return 1 as std::ffi::c_int;
}
pub unsafe extern "C-unwind" fn luaH_webview_go_back(mut L: *mut lua_State) -> gint {
    return webview_history_go(L, -(1 as std::ffi::c_int));
}
pub unsafe extern "C-unwind" fn luaH_webview_go_forward(mut L: *mut lua_State) -> gint {
    return webview_history_go(L, 1 as std::ffi::c_int);
}
pub unsafe extern "C-unwind" fn luaH_webview_set_session_state(
    mut L: *mut lua_State,
...
pub unsafe extern "C-unwind" fn luaH_webview_push_session_state(
    mut L: *mut lua_State,
    mut d: *mut webview_data_t,
) -> std::ffi::c_int {
...
static mut related_view: *mut WebKitWebView = 0 as *const WebKitWebView as *mut WebKitWebView;
static mut last_popup: C2RustUnnamed_3 = {
    let init = C2RustUnnamed_3 {
        refs: std::ptr::null_mut(),
        old_refs: std::ptr::null_mut(),
    };
    init
};
...
static mut webview_settings_properties: [property_t; 48] = [
    {
        let init = property_t {
            ...
            name: b"allow-file-access-from-file-urls\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
    ...
    },
    {
        let init = property_t {
            ...
            name: b"default-monospace-font-size\0" as *const u8 as *const std::ffi::c_char,
            type_0: INT,
            writable: GTRUE,
        };
        init
    },
    {
        let init = property_t {
            ...
            name: b"draw-compositing-indicators\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
    },
    {
        let init = property_t {
            ...
            name: b"enable-accelerated-2d-canvas\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
    ...
    },
    {
        let init = property_t {
            ...
            name: b"enable-html5-local-storage\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
    ...
    },
    {
        let init = property_t {
            ...
            name: b"enable-resizable-text-areas\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
    },
    {
        let init = property_t {
            ...
            name: b"enable-site-specific-quirks\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
    ...
    },
    {
        let init = property_t {
            ...
            name: b"javascript-can-access-clipboard\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
    ...
    },
    {
        let init = property_t {
            ...
            name: b"media-playback-allows-inline\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
    },
    {
        let init = property_t {
            ...
            name: b"media-playback-requires-user-gesture\0" as *const u8 as *const std::ffi::c_char,
            type_0: BOOL,
            writable: GTRUE,
        };
        init
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn luaH_checkwebview(mut L: *mut lua_State, mut udx: gint) -> *mut widget_t {
    let mut w = luaH_checkwidget(L, udx);
    if (*(*w).info).tok as std::ffi::c_uint != L_TK_WEBVIEW as std::ffi::c_int as std::ffi::c_uint {
        luaL_argerror(
            L,
            udx,
            b"incorrect widget type (expected webview)\0" as *const u8 as *const std::ffi::c_char,
        );
    }
    return w;
}
...
pub unsafe extern "C" fn webview_get_by_id(mut view_id: guint64) -> *mut widget_t {
    ...
    while i < (*globalconf.webviews).len {
        ...
        if webkit_web_view_get_page_id(g_type_check_instance_cast(
...
unsafe extern "C-unwind" fn luaH_webview_load_string(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    let mut string = luaL_checklstring(L, 2 as std::ffi::c_int, std::ptr::null_mut());
    let mut base_uri = luaL_checklstring(L, 3 as std::ffi::c_int, std::ptr::null_mut());
    webkit_web_view_load_alternate_html((*d).view, string, base_uri, std::ptr::null_mut());
...
unsafe extern "C-unwind" fn luaH_webview_save(mut L: *mut lua_State) -> gint {
    let mut scbs = g_malloc0_n(1, ::core::mem::size_of::<save_cb_s>()) as *mut save_cb_s;
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    (*scbs).filename = luaL_checklstring(L, 2 as std::ffi::c_int, std::ptr::null_mut());
    ...
    webkit_web_view_save_to_file(
        ...
        std::ptr::null_mut(),
        Some(save_cb as unsafe extern "C" fn(*mut GObject, *mut GAsyncResult, gpointer) -> ()),
        scbs as gpointer,
    );
    return 0 as std::ffi::c_int;
}
unsafe extern "C" fn notify_cb(
    ...
    static mut wvprops: *mut GHashTable = std::ptr::null_mut();
...
unsafe extern "C" fn load_failed_cb(
    ...
    let mut L = common.L;
    (*((*w).data as *mut webview_data_t)).is_failed = GTRUE;
    luaH_object_push(L, (*w).ref_0);
...
unsafe extern "C-unwind" fn luaH_webview_push_certificate_flags(
    mut L: *mut lua_State,
    ...
    if errors as std::ffi::c_uint
        & G_TLS_CERTIFICATE_UNKNOWN_CA as std::ffi::c_int as std::ffi::c_uint
        != 0
    {
        lua_pushlstring(
            L,
        ...
        let fresh0 = n;
        n = n + 1;
        lua_rawseti(L, -(2 as std::ffi::c_int), fresh0 as lua_Integer);
    }
    if errors as std::ffi::c_uint
        & G_TLS_CERTIFICATE_BAD_IDENTITY as std::ffi::c_int as std::ffi::c_uint
        != 0
    {
        lua_pushlstring(
            L,
        ...
        let fresh1 = n;
        n = n + 1;
        lua_rawseti(L, -(2 as std::ffi::c_int), fresh1 as lua_Integer);
    }
    if errors as std::ffi::c_uint
        & G_TLS_CERTIFICATE_NOT_ACTIVATED as std::ffi::c_int as std::ffi::c_uint
        != 0
    {
        lua_pushlstring(
            L,
            b"not-activated\0" as *const u8 as *const std::ffi::c_char,
            (::core::mem::size_of::<[std::ffi::c_char; 14]>())
                .wrapping_div(::core::mem::size_of::<std::ffi::c_char>())
                .wrapping_sub(1),
        );
        let fresh2 = n;
        n = n + 1;
        lua_rawseti(L, -(2 as std::ffi::c_int), fresh2 as lua_Integer);
    }
    if errors as std::ffi::c_uint & G_TLS_CERTIFICATE_EXPIRED as std::ffi::c_int as std::ffi::c_uint
        != 0
    {
        lua_pushlstring(
            L,
        ...
        let fresh3 = n;
        n = n + 1;
        lua_rawseti(L, -(2 as std::ffi::c_int), fresh3 as lua_Integer);
    }
    if errors as std::ffi::c_uint & G_TLS_CERTIFICATE_REVOKED as std::ffi::c_int as std::ffi::c_uint
        != 0
    {
        lua_pushlstring(
            L,
        ...
        let fresh4 = n;
        n = n + 1;
        lua_rawseti(L, -(2 as std::ffi::c_int), fresh4 as lua_Integer);
    }
    if errors as std::ffi::c_uint
        & G_TLS_CERTIFICATE_INSECURE as std::ffi::c_int as std::ffi::c_uint
        != 0
    {
        lua_pushlstring(
            L,
        ...
        let fresh5 = n;
        n = n + 1;
        lua_rawseti(L, -(2 as std::ffi::c_int), fresh5 as lua_Integer);
    }
    if errors as std::ffi::c_uint
        & G_TLS_CERTIFICATE_GENERIC_ERROR as std::ffi::c_int as std::ffi::c_uint
        != 0
    {
        lua_pushlstring(
            L,
        ...
        let fresh6 = n;
        n = n + 1;
        lua_rawseti(L, -(2 as std::ffi::c_int), fresh6 as lua_Integer);
    }
    return 1 as std::ffi::c_int;
}
...
unsafe extern "C" fn webview_get_source_finished(
    ...
    let mut length = 0;
    let mut source: *const gchar =
        webkit_web_resource_get_data_finish(main_resource, res, &mut length, std::ptr::null_mut())
            as *mut gchar;
    g_object_unref(main_resource as *mut GObject);
    lua_pushlstring(L, source, length);
    luaH_resume(L, 1 as std::ffi::c_int);
}
unsafe extern "C-unwind" fn luaH_webview_push_source(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    let mut main_resource = webkit_web_view_get_main_resource((*d).view);
    if main_resource.is_null() {
        return 0 as std::ffi::c_int;
    }
    g_object_ref(main_resource as *mut GObject);
...
unsafe extern "C" fn load_changed_cb(
    ...
    match e as std::ffi::c_uint {
        0 => {
            name = b"provisional\0" as *const u8 as *const std::ffi::c_char as *mut gchar;
        }
        1 => {
    ...
    update_uri(w, std::ptr::null_mut());
    if e as std::ffi::c_uint == WEBKIT_LOAD_STARTED as std::ffi::c_int as std::ffi::c_uint {
        (*((*w).data as *mut webview_data_t)).is_committed = GFALSE;
    } else if e as std::ffi::c_uint == WEBKIT_LOAD_COMMITTED as std::ffi::c_int as std::ffi::c_uint
        || e as std::ffi::c_uint == WEBKIT_LOAD_FINISHED as std::ffi::c_int as std::ffi::c_uint
    {
        (*((*w).data as *mut webview_data_t)).is_committed = GTRUE;
    }
    if e as std::ffi::c_uint == WEBKIT_LOAD_STARTED as std::ffi::c_int as std::ffi::c_uint {
        if !((*d).cert).is_null() {
            g_object_unref(g_type_check_instance_cast(
    ...
    } else if e as std::ffi::c_uint == WEBKIT_LOAD_COMMITTED as std::ffi::c_int as std::ffi::c_uint
    {
        ...
        webkit_web_view_get_tls_info((*d).view, &mut (*d).cert, std::ptr::null_mut());
        if !((*d).cert).is_null() {
            g_object_ref(g_type_check_instance_cast(
                (*d).cert as *mut GTypeInstance,
                ((20 as std::ffi::c_int) << 2 as std::ffi::c_int) as GType,
    ...
    if e as std::ffi::c_uint == WEBKIT_LOAD_COMMITTED as std::ffi::c_int as std::ffi::c_uint {
        webview_update_stylesheets(L, w);
    }
    if e as std::ffi::c_uint == WEBKIT_LOAD_STARTED as std::ffi::c_int as std::ffi::c_uint {
        (*((*w).data as *mut webview_data_t)).is_failed = GFALSE;
    }
    if e as std::ffi::c_uint == WEBKIT_LOAD_FINISHED as std::ffi::c_int as std::ffi::c_uint
...
unsafe extern "C" fn create_cb(
    ...
    if related_view.is_null() {
        ...
        g_assertion_message_expr(
            ...
            (*::core::mem::transmute::<&[u8; 10], &[std::ffi::c_char; 10]>(b"create_cb\0"))
                .as_ptr(),
    ...
    if ret != 0 {
        new = luaH_toudata(L, -(1 as std::ffi::c_int), &mut widget_class) as *mut widget_t;
        if !new.is_null() {
    ...
    match type_0 as std::ffi::c_uint {
        0 | 1 => {
            ...
            let mut np = g_type_check_instance_cast(
                ...
                as *mut WebKitNavigationPolicyDecision;
            ...
            let mut uri = webkit_uri_request_get_uri(webkit_navigation_action_get_request(na));
            let mut reason = std::ptr::null_mut();
            match webkit_navigation_action_get_navigation_type(na) as std::ffi::c_uint {
                0 => {
                    reason =
                        b"link-clicked\0" as *const u8 as *const std::ffi::c_char as *mut gchar;
                }
                1 => {
                    reason =
                        b"form-submitted\0" as *const u8 as *const std::ffi::c_char as *mut gchar;
                }
                2 => {
                    reason =
                        b"back-forward\0" as *const u8 as *const std::ffi::c_char as *mut gchar;
                }
                3 => {
                    reason = b"reload\0" as *const u8 as *const std::ffi::c_char as *mut gchar;
                }
                4 => {
                    reason =
                        b"form-resubmitted\0" as *const u8 as *const std::ffi::c_char as *mut gchar;
                }
                5 => {
                    reason = b"other\0" as *const u8 as *const std::ffi::c_char as *mut gchar;
                }
                _ => {
                    _log(
                        LOG_LEVEL_warn,
        ...
        2 => {
            let mut rp = g_type_check_instance_cast(
                ...
                as *mut WebKitResponsePolicyDecision;
            ...
            if webkit_uri_response_get_status_code(r) != 0
                && !(webkit_uri_response_get_status_code(r) >= 200 as std::ffi::c_int as guint
                    && webkit_uri_response_get_status_code(r) < 300 as std::ffi::c_int as guint)
            {
                return GFALSE;
            }
            luaH_object_push(L, (*w).ref_0);
            ...
            let mut ignore_0 =
                (ret_0 != 0 && lua_toboolean(L, -(1 as std::ffi::c_int)) == 0) as std::ffi::c_int;
            if ignore_0 != 0 {
                webkit_policy_decision_ignore(p);
            } else if strcmp(
...
unsafe extern "C-unwind" fn luaH_webview_reload(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    webkit_web_view_reload((*d).view);
    return 0 as std::ffi::c_int;
}
unsafe extern "C-unwind" fn luaH_webview_reload_bypass_cache(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    webkit_web_view_reload_bypass_cache((*d).view);
    return 0 as std::ffi::c_int;
}
unsafe extern "C-unwind" fn luaH_webview_search(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    let mut text = luaL_checklstring(L, 2 as std::ffi::c_int, std::ptr::null_mut());
    ...
    webkit_find_controller_search(
        ...
        (WEBKIT_FIND_OPTIONS_CASE_INSENSITIVE as std::ffi::c_int
            * (case_sensitive == 0) as std::ffi::c_int
            | WEBKIT_FIND_OPTIONS_BACKWARDS as std::ffi::c_int * (forward == 0) as std::ffi::c_int
            | WEBKIT_FIND_OPTIONS_WRAP_AROUND as std::ffi::c_int * wrap) as guint32,
        max_match_count,
    );
    return 0 as std::ffi::c_int;
}
unsafe extern "C-unwind" fn luaH_webview_search_next(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    let mut webkit_fc = webkit_web_view_get_find_controller((*d).view);
    webkit_find_controller_search_next(webkit_fc);
    return 0 as std::ffi::c_int;
}
unsafe extern "C-unwind" fn luaH_webview_search_previous(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    let mut webkit_fc = webkit_web_view_get_find_controller((*d).view);
    webkit_find_controller_search_previous(webkit_fc);
    return 0 as std::ffi::c_int;
}
unsafe extern "C-unwind" fn luaH_webview_clear_search(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    let mut webkit_fc = webkit_web_view_get_find_controller((*d).view);
    webkit_find_controller_search_finish(webkit_fc);
    return 0 as std::ffi::c_int;
}
unsafe extern "C-unwind" fn luaH_webview_loading(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    luaH_gobject_index(
        L,
...
unsafe extern "C-unwind" fn luaH_webview_stop(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    webkit_web_view_stop_loading((*d).view);
    return 0 as std::ffi::c_int;
}
unsafe extern "C-unwind" fn luaH_webview_crash(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
    let mut header = {
    ...
    ipc_send((*d).ipc, &mut header, std::ptr::null_mut());
    return 0 as std::ffi::c_int;
}
unsafe extern "C-unwind" fn luaH_webview_ssl_trusted(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
...
unsafe extern "C-unwind" fn luaH_webview_allow_certificate(mut L: *mut lua_State) -> gint {
    _log(
        LOG_LEVEL_warn,
        b"widgets/webview.c\0" as *const u8 as *const std::ffi::c_char,
...
unsafe extern "C-unwind" fn luaH_webview_push_certificate(
    mut L: *mut lua_State,
    mut w: *mut widget_t,
) -> gint {
    let mut d = (*w).data as *mut webview_data_t;
...
unsafe extern "C" fn webview_translate_old_token(mut token: luakit_token_t) -> luakit_token_t {
...
unsafe extern "C" fn permission_request_cb(
    ...
    }) != 0
    {
        ...
        let mut ummpr = request as *mut WebKitInstallMissingMediaPluginsPermissionRequest;
        lua_pushstring(
            L,
            webkit_install_missing_media_plugins_permission_request_get_description(ummpr),
        );
    } else if ({
        let mut __inst = request as *mut GTypeInstance;
...
unsafe extern "C-unwind" fn luaH_webview_set_pdfjs(mut L: *mut lua_State) -> gint {
    let mut d = (*luaH_checkwebview(L, 1 as std::ffi::c_int)).data as *mut webview_data_t;
...
unsafe extern "C" fn luaH_webview_index(
    ...
    match token as std::ffi::c_uint {
        ...
        27 => {
            lua_pushcclosure(L, luaH_webview_clear_search, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        196 => {
            lua_pushcclosure(L, luaH_webview_search, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        197 => {
            lua_pushcclosure(L, luaH_webview_search_next, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        198 => {
            lua_pushcclosure(L, luaH_webview_search_previous, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        105 => {
            lua_pushcclosure(L, luaH_webview_go_back, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        106 => {
            lua_pushcclosure(L, luaH_webview_go_forward, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        18 => {
            lua_pushcclosure(L, luaH_webview_can_go_back, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        19 => {
            lua_pushcclosure(L, luaH_webview_can_go_forward, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        89 => {
            lua_pushcclosure(L, luaH_webview_eval_js, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        138 => {
            lua_pushcclosure(L, luaH_webview_load_string, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        188 => {
            lua_pushcclosure(L, luaH_webview_save, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        137 => {
            lua_pushcclosure(L, luaH_webview_loading, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        178 => {
            lua_pushcclosure(L, luaH_webview_reload, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        179 => {
            lua_pushcclosure(L, luaH_webview_reload_bypass_cache, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        222 => {
            lua_pushcclosure(L, luaH_webview_ssl_trusted, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        227 => {
            lua_pushcclosure(L, luaH_webview_stop, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        36 => {
            lua_pushcclosure(L, luaH_webview_crash, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        214 => {
            lua_pushcclosure(L, luaH_webview_show_inspector, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        31 => {
            lua_pushcclosure(L, luaH_webview_close_inspector, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        4 => {
            lua_pushcclosure(L, luaH_webview_allow_certificate, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        209 => {
            lua_pushcclosure(L, luaH_webview_set_pdfjs, 0 as std::ffi::c_int);
            return 1 as std::ffi::c_int;
        }
        113 => {
        ...
        217 => {
            return luaL_error(
                L,
                b"view.source has been removed; use view:get_source() instead\0" as *const u8
                    as *const std::ffi::c_char,
            );
        }
        103 => {
            lua_pushcclosure(L, luaH_webview_push_source, 0 as std::ffi::c_int);
    ...
    {
        match webkit_settings_get_hardware_acceleration_policy(webkit_web_view_get_settings(
            (*d).view,
        )) as std::ffi::c_uint
        {
            0 => {
                lua_pushstring(L, b"on-demand\0" as *const u8 as *const std::ffi::c_char);
                return 1 as std::ffi::c_int;
            }
            1 => {
    ...
    return luaL_error(
        L,
        b"cannot get unknown webview property '%s'\0" as *const u8 as *const std::ffi::c_char,
        lua_tolstring(L, 2 as std::ffi::c_int, std::ptr::null_mut()),
    );
}
unsafe extern "C" fn parse_uri(mut uri: *const gchar) -> *mut gchar {
    if uri.is_null()
        || *uri.offset(0 as std::ffi::c_int as isize) == 0
        || g_strcmp0(
            uri,
            b"about:blank\0" as *const u8 as *const std::ffi::c_char,
        ) == 0
    {
        return g_strdup(b"about:blank\0" as *const u8 as *const std::ffi::c_char);
    } else if !(g_strrstr(uri, b"://\0" as *const u8 as *const std::ffi::c_char)).is_null() {
        return g_strdup(uri);
    } else if file_exists(uri) != 0 {
        if g_path_is_absolute(uri) != 0 {
            return g_strdup_printf(b"file://%s\0" as *const u8 as *const std::ffi::c_char, uri);
        } else {
            let mut cwd = g_get_current_dir();
            let mut path = g_build_filename(cwd, uri);
            let mut new =
                g_strdup_printf(b"file://%s\0" as *const u8 as *const std::ffi::c_char, path);
...
unsafe extern "C" fn luaH_webview_newindex(
    ...
    if token as std::ffi::c_uint == L_TK_ZOOM_LEVEL as std::ffi::c_int as std::ffi::c_uint
        && lua_isnumber(L, 3 as std::ffi::c_int) != 0
        && lua_tonumber(L, 3 as std::ffi::c_int) != 1.0f64
    {
        g_object_freeze_notify(g_type_check_instance_cast(
            (*d).view as *mut GTypeInstance,
            ((20 as std::ffi::c_int) << 2 as std::ffi::c_int) as GType,
        ) as *mut std::ffi::c_void as *mut GObject);
        g_object_set(
        ...
        g_object_thaw_notify(g_type_check_instance_cast(
            (*d).view as *mut GTypeInstance,
            ((20 as std::ffi::c_int) << 2 as std::ffi::c_int) as GType,
        ) as *mut std::ffi::c_void as *mut GObject);
    }
    ...
    if token as std::ffi::c_uint
        == L_TK_HARDWARE_ACCELERATION_POLICY as std::ffi::c_int as std::ffi::c_uint
    {
        let mut str = luaL_checklstring(L, 3 as std::ffi::c_int, std::ptr::null_mut());
            ...
            == 0 as std::ffi::c_int
        {
            ...
            return luaL_error(
                L,
                b"invalid value (expected one of 'on-demand', 'always', 'never')\0" as *const u8
                    as *const std::ffi::c_char,
            );
        }
    ...
    return luaL_error(
        L,
        b"cannot set unknown webview property '%s'\0" as *const u8 as *const std::ffi::c_char,
        lua_tolstring(L, 2 as std::ffi::c_int, std::ptr::null_mut()),
    );
}
...
unsafe extern "C" fn mouse_target_changed_cb(
    ...
    if !link.is_null() {
        ...
        (*d).hover = std::ptr::null_mut();
    }
    lua_settop(L, -(1 as std::ffi::c_int) - 1 as std::ffi::c_int);
}
unsafe extern "C-unwind" fn luaH_push_hit_test(
    mut L: *mut lua_State,
    mut UNUSED_v: *mut WebKitWebView,
    mut w: *mut widget_t,
) -> gint {
...
unsafe extern "C" fn webview_button_cb(
    ...
    luaH_push_hit_test(
        L,
        g_type_check_instance_cast(view as *mut GTypeInstance, webkit_web_view_get_type())
            as *mut std::ffi::c_void as *mut WebKitWebView,
        w,
    );
...
unsafe extern "C" fn hide_popup_cb(mut UNUSED_v: *mut WebKitWebView, mut UNUSED_w: *mut widget_t) {
    ...
    if !(last_popup.old_refs).is_null() {
        ...
        last_popup.old_refs = std::ptr::null_mut();
    }
}
...
unsafe extern "C" fn table_from_context_menu(
    ...
    while i <= len {
        ...
        if webkit_context_menu_item_is_separator(item) != 0 {
            ...
            lua_rawseti(L, -(2 as std::ffi::c_int), 1);
            if !submenu.is_null() {
                table_from_context_menu(L, submenu, w);
            } else if stock_action as std::ffi::c_uint
                == WEBKIT_CONTEXT_MENU_ACTION_CUSTOM as std::ffi::c_int as std::ffi::c_uint
            {
                context_menu_actions = g_slist_prepend(context_menu_actions, action as gpointer);
                g_object_ref(g_type_check_instance_cast(
                    action as *mut GTypeInstance,
                    ((20 as std::ffi::c_int) << 2 as std::ffi::c_int) as GType,
                ) as *mut std::ffi::c_void as *mut GObject);
            ...
            lua_rawseti(L, -(2 as std::ffi::c_int), 2);
        }
        lua_rawseti(L, -(2 as std::ffi::c_int), i as lua_Integer);
        i = i.wrapping_add(1);
        i;
    }
    return 1 as std::ffi::c_int;
    ...
    while i <= len {
            ...
            && lua_rawlen(L, -(1 as std::ffi::c_int)) >= 2 as std::ffi::c_int as size_t
        {
            ...
            if lua_type(L, -(1 as std::ffi::c_int)) == LUA_TTABLE {
                ...
                let mut action =
                    gtk_action_new(label, label, std::ptr::null_mut(), std::ptr::null_mut());
                ...
                g_signal_connect_data(
                    action as *mut GObject,
                    b"activate\0" as *const u8 as *const std::ffi::c_char,
                    ::core::mem::transmute::<
                        Option<unsafe extern "C" fn(*mut GtkAction, *mut widget_t) -> ()>,
                        GCallback,
                    >(Some(
                        menu_item_cb as unsafe extern "C" fn(*mut GtkAction, *mut widget_t) -> (),
                    )),
                    w as gpointer,
                    None,
                    G_CONNECT_DEFAULT,
                ...
                let mut stock_action =
                    lua_tointeger(L, -(1 as std::ffi::c_int)) as WebKitContextMenuAction;
                let mut __n1 = stock_action as gint64;
                let mut __n2 = WEBKIT_CONTEXT_MENU_ACTION_CUSTOM as std::ffi::c_int as gint64;
                if !(__n1 != __n2) {
                    g_assertion_message_cmpint(
                        ...
                        .as_ptr(),
                        b"stock_action != WEBKIT_CONTEXT_MENU_ACTION_CUSTOM\0" as *const u8
                            as *const std::ffi::c_char,
                        __n1 as guint64,
                        b"!=\0" as *const u8 as *const std::ffi::c_char,
                        __n2 as guint64,
...
unsafe extern "C" fn context_menu_cb(
    ...
    let mut L = common.L;
    if context_menu_actions.is_null() {
    } else {
        g_assertion_message_expr(
            G_LOG_DOMAIN as *const std::ffi::c_char,
            b"widgets/webview.c\0" as *const u8 as *const std::ffi::c_char,
            1244 as std::ffi::c_int,
            (*::core::mem::transmute::<&[u8; 16], &[std::ffi::c_char; 16]>(b"context_menu_cb\0"))
                .as_ptr(),
    ...
    g_slist_free_full(context_menu_actions, Some(g_object_unref_c));
    context_menu_actions = std::ptr::null_mut();
    lua_settop(L, -(1 as std::ffi::c_int) - 1 as std::ffi::c_int);
    return GFALSE;
}

unsafe extern "C" fn g_object_unref_c(ptr: *mut libc::c_void) {
    g_object_unref(ptr as *mut GObject)
}
...
unsafe extern "C" fn webview_destructor(mut w: *mut widget_t) {
    ...
    g_slice_free1(::core::mem::size_of::<webview_data_t>(), d as gpointer);
}
#[unsafe(no_mangle)]
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn webview_get_endpoint(mut w: *mut widget_t) -> *mut ipc_endpoint_t {
    if (*(*w).info).tok as std::ffi::c_uint == L_TK_WEBVIEW as std::ffi::c_int as std::ffi::c_uint {
    } else {
        g_assertion_message_expr(
            G_LOG_DOMAIN as *const std::ffi::c_char,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn webview_set_web_process_id(mut w: *mut widget_t, mut pid: pid_t) {
    let mut d = (*w).data as *mut webview_data_t;
...
pub unsafe extern "C" fn widget_webview(
    ...
    (*w).index = Some(luaH_webview_index);
    (*w).newindex = Some(luaH_webview_newindex);
    (*w).destructor = Some(webview_destructor as unsafe extern "C" fn(*mut widget_t) -> ());
    let mut d = g_slice_alloc0(::core::mem::size_of::<webview_data_t>()) as *mut webview_data_t;
    (*d).widget = w;
    (*w).data = d as gpointer;
    let mut prop_tbl_idx = luaH_absindex(L, -(4 as std::ffi::c_int));
    if lua_type(L, prop_tbl_idx) == 5 as std::ffi::c_int {
    } else {
        g_assertion_message_expr(
            G_LOG_DOMAIN as *const std::ffi::c_char,
            b"widgets/webview.c\0" as *const u8 as *const std::ffi::c_char,
            1385 as std::ffi::c_int,
            (*::core::mem::transmute::<&[u8; 15], &[std::ffi::c_char; 15]>(b"widget_webview\0"))
                .as_ptr(),
    ...
    (*d).stylesheets = std::ptr::null_mut();
    ...
    (*d).is_committed = GFALSE;
    (*d).ipc = ipc_endpoint_new(b"UI\0" as *const u8 as *const std::ffi::c_char);
    (*w).widget = g_type_check_instance_cast((*d).view as *mut GTypeInstance, gtk_widget_get_type())
        as *mut std::ffi::c_void as *mut GtkWidget;
    ...
    g_object_connect(
        ...
        ::core::mem::transmute::<
            Option<unsafe extern "C" fn(*mut WebKitWebView, *mut widget_t) -> gboolean>,
            GCallback,
        >(Some(
            webview_crashed_cb
                as unsafe extern "C" fn(*mut WebKitWebView, *mut widget_t) -> gboolean,
        )),
        w,
        ...
        ::core::mem::transmute::<
            Option<unsafe extern "C" fn(*mut WebKitWebView, *mut GParamSpec, *mut widget_t) -> ()>,
            GCallback,
        >(Some(
            notify_cb
                as unsafe extern "C" fn(*mut WebKitWebView, *mut GParamSpec, *mut widget_t) -> (),
        )),
        w,
        b"signal::load-changed\0" as *const u8 as *const std::ffi::c_char,
        ::core::mem::transmute::<
            Option<unsafe extern "C" fn(*mut WebKitWebView, WebKitLoadEvent, *mut widget_t) -> ()>,
            GCallback,
        >(Some(
            load_changed_cb
                as unsafe extern "C" fn(*mut WebKitWebView, WebKitLoadEvent, *mut widget_t) -> (),
        )),
        w,
        ...
        ::core::mem::transmute::<
            ...
            GCallback,
        >(Some(
            hide_popup_cb as unsafe extern "C" fn(*mut WebKitWebView, *mut widget_t) -> (),
        )),
        w,
    ...
    g_object_connect(
        ...
        ::core::mem::transmute::<
            Option<unsafe extern "C" fn(*mut WebKitFindController, guint, *mut widget_t) -> ()>,
            GCallback,
        >(Some(
            found_text_cb
                as unsafe extern "C" fn(*mut WebKitFindController, guint, *mut widget_t) -> (),
        )),
        w,
        b"signal::failed-to-find-text\0" as *const u8 as *const std::ffi::c_char,
        ::core::mem::transmute::<
            Option<unsafe extern "C" fn(*mut WebKitFindController, *mut widget_t) -> ()>,
            GCallback,
        >(Some(
            failed_to_find_text_cb
                as unsafe extern "C" fn(*mut WebKitFindController, *mut widget_t) -> (),
        )),
        w,
        // std::ptr::null_mut(),
    );
    ...
    g_object_connect(
        ...
        ::core::mem::transmute::<
            Option<unsafe extern "C" fn(*mut WebKitWebInspector, *mut widget_t) -> gboolean>,
            GCallback,
        >(Some(
            inspector_attach_window_cb
                as unsafe extern "C" fn(*mut WebKitWebInspector, *mut widget_t) -> gboolean,
        )),
        w,
    ...
    gtk_widget_show(g_type_check_instance_cast(
        (*d).view as *mut GTypeInstance,
        gtk_widget_get_type(),
    ) as *mut std::ffi::c_void as *mut GtkWidget);
    return w;
